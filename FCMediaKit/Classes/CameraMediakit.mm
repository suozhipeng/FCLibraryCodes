...
@implementation CameraMediakit


MyCallbackFunction resultCallback;

// 单例的静态实例变量
static CameraMediakit *sharedInstance = nil;

+ (instancetype)sharedInstance {
    // 使用 dispatch_once_t 确保线程安全
...
-(void)setup:(MyCallbackFunction)callback{
    ...
    [self handleRTPWithZLMediaKit:8554];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(receiveCallbackNotification:) name:@"YourCallbackNotification" object:nil];
    resultCallback = callback;
}
- (void)receiveCallbackNotification:(NSNotification *)notification {
    NSString *resultString = notification.object;
    // NSLog(@"OC 函数 收到数据: %@", resultString);
    // 在这里处理结果
...
// 定义C回调函数
void myCallback123(const char *result) {
    NSString *resultString = [NSString stringWithUTF8String:result];
    //     发送NSNotification
    [[NSNotificationCenter defaultCenter] postNotificationName:@"YourCallbackNotification" object:resultString];
}

/// 使用 SDK 处理 rtp 数据
/// @param rtspPort 转换后的 rtsp 端口号:  输入 0 或者-1, 设置为 默认 8554
-(void)handleRTPWithZLMediaKit:(int)rtspPort {
...
//       mk_http_server_start(443, 1);
    int tmpRTSPPort = rtspPort < 1 ? 8554 : rtspPort;
       mk_rtsp_server_start(tmpRTSPPort, 0);
...
void API_CALL on_mk_media_publish(const mk_media_info url_info,
    ...
    // 传递出去的数据 2024-01-06
    char url[256];
    snprintf(url,256,"%s/%s", mk_media_info_get_app(url_info),mk_media_info_get_stream(url_info));
    myCallback123(url);
